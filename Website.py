...
def initialize_mongo(debug_mode=False):
    ...
    for attempt in range(3):  # Retry 3 times
        try:
            client = MongoClient(MONGO_URL, serverSelectionTimeoutMS=30000, tls=True, tlsAllowInvalidCertificates=False)
...
mongo_client, db, users_collection, applications_collection = initialize_mongo(debug_mode=True)
...
def save_user(email, name, password):
    ...
    try:
        ...
        st.error(f"An error occurred百分之 while saving user: {e}")
