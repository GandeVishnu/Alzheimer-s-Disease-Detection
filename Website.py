...
def generate_pdf(name, age, place, phone_number, image_path, diagnosis, confidence):
    ...
    # Sanitize the patient name for the filename (replace spaces with underscores and remove non-alphanumeric characters)
    safe_name = re.sub(r'[^a-zA-Z0-9_]', '', name.replace(' ', '_'))
    pdf_filename = f"{safe_name}_Report.pdf"  # Use sanitized name for the PDF filename
    ...
    pdf.cell(200, 10, txt=f"Report Generated: {current_datetime}", ln=True)
    ...
    # Save the PDF with the patient's name as the filename
